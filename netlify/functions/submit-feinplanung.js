...
function parseMultipart(event){
  return new Promise((resolve, reject) => {
    ...
    const bb = Busboy({ headers, limits: { fileSize: 25 * 1024 * 1024, files: 50 } });
    const fields = {};
    const files = {};
...
function buildFullSummary(fields, files){
  const baseSummary = fields.zusammenfassung || '';
  const uploadSummary = `
### Uploads / Dateinamen
...
Sonstige: ${fileText(files,'bild_sonstige')}
`;
  return `${baseSummary}\n\n${uploadSummary}`.trim();
}
...
function appendText(formData, questions, fields, htmlName, titles){
  ...
  if (q && q.name && value && String(value).trim()) {
    formData.append(q.name, value);
  }
}
...
function appendFiles(formData, questions, files, htmlName, titles){
  ...
  for (const f of uploaded) {
    ...
    // Jotform file fields usually accept the field name, and for multiple files also name[].
    // Append both styles defensively only if needed? Use array-style first.
...
exports.handler = async (event) => {
  try{
    ...
    if (nameQ && nameQ.name && fields.kundenname) {
      formData.append(`${nameQ.name}[first]`, fields.kundenname);
    }

    const summary = buildFullSummary(fields, files);
    const summaryQ =
    ...
    formData.append(summaryQ.name, summary);

    // Real image/file uploads to Jotform file fields
    ...
    const submitRes = await fetch(`https://submit.jotform.com/submit/${FORM_ID}/`, {
      ...
      body: formData,
      headers: {
        'User-Agent': 'Mozilla/5.0 Zukunftssolar-Netlify-File-Submit'
      }
    });
    ...
    if (!submitRes.ok) {
      return {
        statusCode: 500,
        body: JSON.stringify({ success:false, message:`Jotform Submit Fehler ${submitRes.status}: ${responseText.slice(0, 250)}` })
      };
    }

    return {
      statusCode: 200,
      body: JSON.stringify({ success:true, message:'Feinplanung inklusive Bilder wurde an Jotform gesendet' })
    };
  }catch(err){
    return {
      statusCode: 500,
      body: JSON.stringify({ success:false, message: err.message || 'Unbekannter Serverfehler' })
    };
  }
};
